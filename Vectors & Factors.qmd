...
(which is, unsurprisingly, called `barplot()`) needs at the very
least ...

-   a `height` argument, i.e., our y-axis values and

-   a `names.arg` argument, i.e., our x-axis label.
...
In RStudio, you now have the option to save the plot to your computer. Once the figure has appeared in your "Plots" panel, you can click on "Export" in the menu bar below and proceed to choose the desired output format file directory.

## Some technical details

The example above demonstrates one of the most important data structures in R:
**Vectors**. They form the cornerstone of various, more complex objects such as data frames, and are essential to handling large data sets (e.g., corpora). And yet, vectors are very simple in that they merely constitute one-dimensional sequences of characters or numbers -- no more, no less.
...
The individual elements in these two vectors are not randomly jumbling around in virtual space,
but are in fact following a clear order. Each element has an "ID" (or
**index**), by which we can access it. For example, if we want to print
the first lemma in our `lemma` variable, we can use this notation:
...
4.  Once we are sure that an element is in the vector of interest,
    ...
    element. You can read this notation as "Which element in `pp3` is
    *they*?". The output suggests that is in position `6`. Note that the
    number obtained depends on the order of elements you've chosen when
    creating `pp3`.
