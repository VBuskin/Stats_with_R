...
Please download the file [`Paquot_Larsson_2020_data.xlsx`](https://osf.io/eu4gw)
[@paquotDescriptiveStatisticsVisualization2020] and store
...
In order to investigate one or more linguistic features, we
...
Since it is not feasible to examine, for instance, the entirety of all linguistic
utterances ever produced, i.e., the virtually infinite **population**, we rely on subsets of it: **samples**. Good research is characterised by good **sampling procedures** that limit the **bias** present in any sample, thus improving the generalisability of potential findings. 
...
$\omega \in \Omega$, $X$ has a value $x$. The set $S$
comprises all possible unique values of $X$, e.g.,
...
a **dataset**, which is essentially a matrix with **rows** (= observations) and **columns** (= variables). Consider the `head()` of the clause data:
...
Assuming that $n$ denotes the row number and
$p$ the column number, the abstract form of a dataset is an $n \times p$ matrix of the form in @eq-dataset.
