...
observed frequencies (actual data) with expected frequencies (assuming
the null hypothesis were true). The $\chi^2$ (chi-squared) score
quantifies the difference between them for every cell in the table
according to this simplified formula:

$$
\text{chi-squared =}\frac{(observed - expected)^2}{expected}
$$ {#eq-chisq-simple}

The greater the difference between observed and expected, the higher the
$\chi^2$ score, the more evidence we obtain against $H_0$. We should
always report the following three elements:
...
(for $1, ..., n$) in our data set and the **expected frequencies**
$m_{ij}$ (for $1, ..., m$), which we would expect to see if $H_0$ were
true. The indices $i$ and $j$ uniquely identify the frequencies found in
...
The outcome varies between $0$ (= no correlation) and $1$ (= perfect
correlation); cf. also Gries [-@gries_statistics_2013: 186].
