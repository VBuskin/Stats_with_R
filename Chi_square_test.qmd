...
independence would entail that the probabilities of the outcomes of the response variable
`ORDER = "mc-sc"` and `ORDER = "sc-mc"` are not influenced by whether they occur in the groups `SUBORDTYPE = "temp"` or `SUBORDTYPE = "caus"`.
...
robust test such as the **Fisher's Exact Test** or the **log-likelihood ratio test** ($G$-test).
