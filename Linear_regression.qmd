...
contribution of $X$ to the explanation of $Y$, $\epsilon$ describes the
errors of the model.

## Linear Regression

-   Linear regression is a simple approach to supervised machine
    learning where the response variable is known. It assumes that the
    dependence of $Y$ on $X$ is **linear**.

-   This approach is suited for **numerical response variables**. The
    predictors can be either numerical or discrete/categorical.
...
the **intercept** and **slope**, respectively. Together they are
referred to as the model **coefficients** (or parameters), and
$\epsilon$ is the error term. The fact that assumptions are made about
the form of the model renders it a **parametric model**.
...
-   Given the existing data on $X$ and $Y$, which is also known as the
    ...
    coefficients, we cannot know the true values of the coefficients,
    i.e., the exact true relationship between the variables.

-   The most common way of estimating parameters for linear models is
    the **least squares** approach. In essence, the parameters are
    chosen such that the residual sum of squares[^linear_regression-1],
    i.e., the sum of the differences between observed and predicted
    values, is as low as possible.

-   We can then predict future sales using the formula

[^linear_regression-1]: $$ \hat{\beta}_1 = \frac{\sum_{i=1}^n(x_i - \bar{x})(y_i- \bar{y})}{\sum_{i=1}(x_i- \bar{x})^2}
    $$
...
predictor values $X = x$. The **hat** symbol \^ marks estimated values.
...
2.  **Residuals**: These indicate the difference between the observed
...
3.  **Coefficients**: The regression coefficients correspond to
...
4.  $p$**-values and** $t$**-statistic**: Given the null hypothesis
...
[^linear_regression-2]: $$t = \frac{\hat{\beta_1} - 0}{SE(\hat{\beta_0})}$$
...
5.  The **residual standard error** (RSE)[^linear_regression-3] is an
    estimation of the average deviation from the observed values.

6.  $R^2$[^linear_regression-4] is important for assessing model fit
...
7.  The $F$-statistic is used to measure the association between the
    dependent variable and the independent variable(s). Generally
    speaking, values greater than 1 indicate a possible correlation. A
    very low $p$-value suggests that the null hypothesis
    $H_0: \beta_1 = 0$ can be rejected.

[^linear_regression-3]: $$RSE = \sqrt{\frac{1}{n-2}\sum_{i=1}^n (y_i - \hat{y_i}})^2$$

[^linear_regression-4]: $$R^2 = 1-\frac{\rm{TSS}}{RSS} = 1-\frac{\sum_{i=1}^n (y_i - \hat{y_i})^2}{\sum_{i=1}^n (y_i - \bar{y_i})^2}$$.
