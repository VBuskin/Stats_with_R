...
Here, a few tweaks are necessary. In general, the `ggplot()` function prefers to works with data frames rather than cross-tables, so we'll have to
coerce it into one first:
