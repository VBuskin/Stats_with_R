...
variance in the data. This is also known as **dimension reduction**, which allows researchers to see overarching
...
combinations of them, yielding $k$ principal components $Z_k$ (PCs hf.) of the
dataset (for $k \in \{1, ..., K\}$). PCs are ordered such that the first PC explains the most variance in the data, with each subsequent PC explaining the maximum remaining variance while being uncorrelated with previous PCs. 
